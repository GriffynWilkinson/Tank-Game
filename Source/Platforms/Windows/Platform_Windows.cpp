...
namespace GameDev2D
{
    ...
    void Platform_Windows::Draw()
    {
        ...
        if (m_Game != nullptr)
        {
            //Safety check the camera
            if (m_Game->GetCamera() == nullptr)
            {
                Error(false, "Warning - The Game's Camera is null - make sure to create a Camera object in the Game's constructor");
            }

            //Draw the game
            //Cache the active camera and set the game's camera as the active camera
            Camera* activeCamera = ServiceLocator::GetGraphics()->GetActiveCamera();
            ServiceLocator::GetGraphics()->SetActiveCamera(m_Game->GetCamera());
    ...
    void Platform_Windows::HandleViewportResize(unsigned int aWidth, unsigned int aHeight)
    {
        ...
        if(aWidth != m_Width || aHeight != m_Height)
        {
            ...
            Event* event = new ResizeEvent(vec2(m_Width, m_Height));
    ...
    void Platform_Windows::ApplyVideoModeChanges(VideoModeInfo* aVideoModeInfo)
    {
        ...
        {
            Event* event = new FullscreenEvent(m_IsFullScreen);
            m_Game->HandleEvent(event);
            DispatchEvent(event);
        }
    }
...
#pragma warning(pop)
    ...
    double Platform_Windows::GetTicks()
    {
	    LARGE_INTEGER largeInt;
	    QueryPerformanceCounter(&largeInt);
        double ticks = (double)(largeInt.QuadPart) / m_PerformanceCounterFrequency;
	    return ticks;
    }
}