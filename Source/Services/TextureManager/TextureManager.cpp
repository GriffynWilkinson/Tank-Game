...
namespace GameDev2D
{
    ...
    void TextureManager::AddTexture(const string& aFilename, TextureFrame* aTextureFrame)
    {
        ...
        string path = ServiceLocator::GetPlatformLayer()->GetPathForResourceInDirectory(filename.c_str(), "png", "Images");
        ...
        if(texturePair.first == nullptr)
        {
            ...
            bool success = PngLoader::LoadFromPath(path, &textureData);

            //Did the image load successfully
            if (success == true && textureData != nullptr)
            {
                //Create a new texture object, set the retain count to 1
                texturePair.first = new Texture(textureData);
                texturePair.second = 1;
            }
            else
            {
                //Delete the texture data (if it exists)
                SafeDelete(textureData);
    ...
    SourceFrame* TextureManager::LoadAtlasFrame(const string& aFilename, const string& aAtlasKey)
    {
        //Get the pair from the texture map
        ...
        if (sourceFrame == nullptr)
        {
            ...
            //Get the json path
            string jsonPath = ServiceLocator::GetPlatformLayer()->GetPathForResourceInDirectory(filename.c_str(), "json", "Images");
    ...
    Texture* TextureManager::GetPlaceHolder()
    {
        if(m_PlaceHolder == nullptr)
        {
            ...
            m_PlaceHolder = new Texture(new TextureData(TextureFormat_RGBA, placeHolderSize, placeHolderSize, data));
        }
