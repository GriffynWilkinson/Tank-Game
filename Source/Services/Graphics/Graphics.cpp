...
namespace GameDev2D
{
    ...
    void Graphics::BindTexture(unsigned int aTextureId)
    {
        //Bind the texture id (if its not aleady bound)
        if (aTextureId != m_BoundTextureId)
        {
            m_BoundTextureId = aTextureId;
    ...
    void Graphics::GenerateTexture(unsigned int* aTextureId)
    {
        glGenTextures(1, aTextureId);
    }

    void Graphics::DeleteTexture(unsigned int* aTextureId)
    {
    ...
    void Graphics::SetTextureData(unsigned int aTextureId, TextureData* aTextureData, unsigned int aMipmapLevel)
    {
        //Safety check that there's actually data to set
        if (aTextureData->width > 0 && aTextureData->height > 0)
        {
            //Bind the texture by its id
            BindTexture(aTextureId);
            ...
            //Handle the OpenGL formats
            unsigned int formats[] = { GL_RGB, GL_RGBA };
            unsigned int format = formats[aTextureData->format];

            //Specify a 2D texture image, provideing the a pointer to the image data in memory
            glTexImage2D(GL_TEXTURE_2D, aMipmapLevel, format, aTextureData->width, aTextureData->height, 0, format, GL_UNSIGNED_BYTE, aTextureData->data);
    ...
    {
        unsigned int renderModes[] = { GL_POINTS, GL_LINES, GL_LINE_LOOP, GL_LINE_STRIP, GL_TRIANGLES, GL_TRIANGLE_STRIP, GL_QUADS, GL_QUAD_STRIP, GL_POLYGON };
