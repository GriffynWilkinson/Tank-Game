...
namespace GameDev2D
{
...
#if DRAW_DEBUG_FPS_COUNTER
        WatchValueUnsignedInt(ServiceLocator::GetPlatformLayer(), CastDebugCallbackUnsignedInt(PlatformLayer::GetFramesPerSecond));
#endif

#if DRAW_DEBUG_DELTA_TIME
        WatchValueDouble(ServiceLocator::GetPlatformLayer(), CastDebugCallbackDouble(PlatformLayer::GetDeltaTime));
#endif

#if DRAW_DEBUG_ELAPSED_TIME
        WatchValueDouble(ServiceLocator::GetPlatformLayer(), CastDebugCallbackDouble(PlatformLayer::GetElapsedTime));
#endif
    ...
    { 
    #if (DRAW_DEBUG_FPS_COUNTER || DRAW_DEBUG_DELTA_TIME) && DEBUG
        return true;
    #else
        return false;
    #endif
    ...
    void DebugUI::UpdateLayout()
    {
        ...
        for(unsigned int i = 0; i < m_Labels.size(); i++)
        {
            m_Labels.at(i).label->SetPosition(position.x, position.y);
            m_Labels.at(i).label->ModelMatrixIsDirty();
