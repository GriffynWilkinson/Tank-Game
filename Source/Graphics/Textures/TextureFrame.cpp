...
namespace GameDev2D
{
    TextureFrame::TextureFrame(const string& aFilename, const string& aAtlasKey) : BaseObject("TextureFrame"),
        m_Texture(nullptr),
        m_Shader(nullptr),
        ...
        m_ColorsBufferObject(0)
    {
        ...
        ServiceLocator::GetTextureManager()->AddTexture(aFilename, this);
        
        //Safety check the atlas key
        if (aAtlasKey != "")
        {
            //Load the atlas frame
            SourceFrame* sourceFrame = ServiceLocator::GetTextureManager()->LoadAtlasFrame(aFilename, aAtlasKey);
        ...
        else
        {
            ...
            m_SourceFrame.size = vec2(m_Texture->GetWidth(), m_Texture->GetHeight());
        }
    }
    ...
    TextureFrame::TextureFrame(Texture* aTexture, bool aOwnsTexture) : BaseObject("TextureFrame"),
        ...
        m_Shader(nullptr),
        m_Color(Color::WhiteColor()),
        m_SourceFrame(SourceFrame(0.0f, 0.0f, m_Texture->GetWidth(), m_Texture->GetHeight())),
    ...
    void TextureFrame::Draw(mat4 aModelMatrix)
    {
        ...
        //Bind the texture
        graphics->BindTexture(GetTexture()->GetId());
        
        //Enable blending if the texture has an alpha channel
        if(GetFormat() == TextureFormat_RGBA || m_Color.Alpha() != 1.0f)
        {
    ...
    TextureFormat TextureFrame::GetFormat()
    {
        if(m_Texture != nullptr)
        {
            return m_Texture->GetFormat();
        }
        return TextureFormat_RGBA; //TODO:
    }
    ...
    void TextureFrame::UpdateVertexBuffer(unsigned int aBufferObjectType)
    {
        ...
        if((aBufferObjectType & BufferObjectUvCoordinates) == BufferObjectUvCoordinates)
        {
            ...
            float x1 = (float)m_SourceFrame.position.x / (float)m_Texture->GetWidth();
            float y1 = 1.0f - (((float)m_SourceFrame.position.y + m_SourceFrame.size.y) / (float)m_Texture->GetHeight());
            float x2 = (float)(m_SourceFrame.position.x + m_SourceFrame.size.x) / (float)m_Texture->GetWidth();
            float y2 = 1.0f - (m_SourceFrame.position.y / (float)m_Texture->GetHeight());
