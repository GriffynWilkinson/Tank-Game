...
namespace GameDev2D
{
        ...
        m_RenderTarget(nullptr)
    {
        ...
        //Get the json path
        string path = ServiceLocator::GetPlatformLayer()->GetPathForResourceInDirectory(filename.c_str(), aExtension.c_str(), "Fonts");
    ...
    void TrueTypeFont::BuildFontAtlas()
    {
        ...
        if(m_RenderTarget != nullptr)
        {
            ...
            for(unsigned int i = 0; i < m_CharacterSet.size(); i++)
            {
                ...
                Texture texture(new TextureData(TextureFormat_RGBA, m_GlyphData[character]->width, m_GlyphData[character]->height, rgbaData));
